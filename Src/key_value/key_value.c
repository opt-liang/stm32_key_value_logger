...
void init_key_value( uint8_t key_value_int32_flash_index, uint8_t key_value_string_flash_index, uint8_t key_value_backup_flash_index ){
    ...
    if( ( array[0] != 0 && ( array[0] + SECTOR_NUM * KEY_VALUE_SIZE) > array[1] ) || ( array[1] + SECTOR_NUM * KEY_VALUE_SIZE) > array[2] ){
        log_e("System allocation error exists cross flash memory\r\n");
        while( true );
    }
        ...
        if( key_value_SemaphoreHandle == NULL ){
            log_e("init_key_value key_value_SemaphoreHandle failed\r\n");
            while( true );
        }
    #endif
...
int aphash( char* str ){
    ...
    if( len > 64 ){
        log_w( "aphash String over 64 bytes\r\n");
    }
...
void check_hash_conflict( int count,...){
    ...
    if( count >= 200 ){
        log_w( "check_hash_conflict--The number is greater than 255\r\n");
        return;
    }
    ...
    char *s;
        ...
        if( array == NULL ){
            log_e( "Allocated memory failure\r\n");
            return;
        }
    #else
    ...
    if ( check_repetition( array, count) ){
        log_e( "Duplication of data\r\n");
        while( true );
    }else{
        log_d( "No Duplication of data\r\n");
    }
    #if SYS
        vPortFree( array );
    #endif
    ...
    if( key_address ){
        if( type == UINT32 ){
            if( KEY_VALUE_INT32 == 0x00 ){
                log_a("GET:UINT32 No initialization\r\n");
                while( true );
            }
            *((uint32_t *)value) = *( key_address + 1 );
        }else if( type == STRINGS ){
            if( KEY_VALUE_STRINGS == 0x00 ){
                log_a("GET:STRINGS No initialization\r\n");
                while( true );
            }
...
bool set_key_value( char *key, enum TYPE type, uint8_t *value ){
    ...
    if( type == UINT32 ){
        ...
        if( KEY_VALUE_INT32 == 0x00 || KEY_VALUE_BACKUP ==0x00 ){
            log_a("UINT32 No initialization\r\n");
            while( true );
        }
        ...
        if( addr ){
			...
			if( move_key_value( type ) == false ){
                log_e("UINT32 write to backup failed\r\n");
				goto exit;
			}
			
            if( cycleCount ++ > 5 ){
                cycleCount = 0;
                log_e( "UINT32 set_key_value backup failed\r\n" );
                goto exit;
            }
            goto int32_rewrite;
        }
        ...
        if( KEY_VALUE_STRINGS == 0x00 || KEY_VALUE_BACKUP ==0x00 ){
            log_a("STRINGS No initialization\r\n");
            while( true );
        }
        ...
        if( len > 256 ){
            log_e( "key_value String over 256 bytes\r\n" );
            return false;
        }
        strings_rewrite:
        addr = __find_key( ERASURE_STATE, type );
        if( addr && ( uint32_t )( addr + 2 + len / 4 + 1 ) < ( KEY_VALUE_STRINGS + KEY_VALUE_MAX_SIZE ) ){
            ...
            if( key_address && addr != (key_address-1) ){   //memcmp( (uint8_t *)(key_address+1), value, len ) ){
                ...
                if( ( uint32_t )( key_address + i ) >= ( KEY_VALUE_STRINGS + KEY_VALUE_MAX_SIZE ) ){
                    log_e( "set_key_value Abnormality error\r\n" );
                    goto exit;
                }else{
                    goto STRINGS_CHECK;
                }
            ...
            //write to backup
			if( move_key_value( type ) == false ){
                log_e("STRINGS write to backup failed\r\n");
				goto exit;
			}
            
            if( cycleCount ++ > 5 ){
                cycleCount = 0;
                log_e( "STRINGS set_key_value backup failed\r\n" );
                goto exit;
            }
